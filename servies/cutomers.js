...
const getById=(id)=>{
    return new Promise((resolve,reject)=>{
        db.query('SELECT * from customers where customer_id = ?',id, (err, rows) => {
            if(err) reject(err);
            resolve(rows)
        });
    })
...
const Post=(data)=>{
    return new Promise((resolve,reject)=>{
        db.query('INSERT INTO customers SET ? ',data, (err, rows) => {
            if(err) reject(err);
            resolve(rows)
        });
    })
...
const deleteById=(id)=>{
    return new Promise((resolve,reject)=>{
        db.query('DELETE FROM customers WHERE customer_id = ?',id, (err, rows) => {
            if(err) reject(err);
            resolve(rows)
        });
    })
